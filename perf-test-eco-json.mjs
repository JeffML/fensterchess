...
async function runTests() {
  ...
  const fenster3 = measure('fensterchess     ', () => {
    return TEST_FENS.map(fen => fensterFindOpening(fenster1.result, fen, fenster2.result, null, null));
  });
  
  console.log(`  Speedup: ${(fenster3.duration / eco3.duration).toFixed(2)}x`);
  console.log(`  Avg per lookup: ${formatTime(eco3.duration / TEST_FENS.length)} (eco.json)`);
  console.log(`  Avg per lookup: ${formatTime(fenster3.duration / TEST_FENS.length)} (fensterchess)`);
  console.log();
  ...
  console.log('Summary');
  console.log('='.repeat(70));
  console.log(`Total time (eco.json):    ${formatTime(eco1.duration + eco2.duration + eco3.duration)}`);
  console.log(`Total time (fensterchess): ${formatTime(fenster1.duration + fenster2.duration + fenster3.duration)}`);
  console.log();
  console.log('Notes:');
