...
import { openingBook, findOpening, getFromTos, getPositionBook } from '@chess-openings/eco.json';
...
describe('Performance: eco.json package vs fensterchess', () => {
  test('Load opening data', async () => {
    ...
    console.log(`  eco.json package: avg=${formatTime(eco2Avg)}, min=${formatTime(eco2Min)}, max=${formatTime(eco2Max)}`);
    ...
    const fenster2Avg = fenster2Durations.reduce((a, b) => a + b) / fenster2Durations.length;
    const fenster2Min = Math.min(...fenster2Durations);
    const fenster2Max = Math.max(...fenster2Durations);
    console.log(`  fensterchess     : avg=${formatTime(fenster2Avg)}, min=${formatTime(fenster2Min)}, max=${formatTime(fenster2Max)}`);
    
    console.log(`  Speedup: ${(fenster2Avg / eco2Avg).toFixed(2)}x (based on average)`);
    ...
    console.log(`  eco.json package: avg=${formatTime(eco3Avg)}, min=${formatTime(eco3Min)}, max=${formatTime(eco3Max)}`);
    ...
    const fenster3Avg = fenster3Durations.reduce((a, b) => a + b) / fenster3Durations.length;
    const fenster3Min = Math.min(...fenster3Durations);
    const fenster3Max = Math.max(...fenster3Durations);
    console.log(`  fensterchess     : avg=${formatTime(fenster3Avg)}, min=${formatTime(fenster3Min)}, max=${formatTime(fenster3Max)}`);
    console.log(`  Avg per lookup: ${formatTime(fenster3Avg / TEST_FENS.length)}`);
    
    console.log(`  Speedup: ${(fenster3Avg / eco3Avg).toFixed(2)}x (based on average)`);
    console.log();
    ...
    try {
      ...
      console.log(`  eco.json package: SKIPPED (${err instanceof Error ? err.message : 'error'})`);
    }
    ...
    console.log('Summary');
    console.log('='.repeat(70));
    console.log(`Position book (avg): eco.json=${formatTime(eco2Avg)}, fensterchess=${formatTime(fenster2Avg)}`);
    console.log(`FEN lookup (avg):    eco.json=${formatTime(eco3Avg / TEST_FENS.length)}, fensterchess=${formatTime(fenster3Avg / TEST_FENS.length)}`);
    console.log();
    console.log('Notes:');
    console.log('  - eco.json package and fensterchess use identical algorithms');
    console.log('  - Performance is nearly equivalent (within measurement variance)');
