...
import { openingBook as getOpeningBook, getPositionBook } from "@chess-openings/eco.json";
...
describe("lookupByMoves with wcup25.pgn games", () => {
  ...
  beforeAll(async () => {
    ...
    positionBook = getPositionBook(openingBook);
  });
...
Kf1 Bxh1 16. Bxf5 Bc5 17. Qg5 Qd1#`;
    ...
    const result = findNearestOpening(moves, openingBook, positionBook);
...
41. h5 b3 42. Qb7 Qc2`;
    ...
    expect(result.movesBack).toBeGreaterThan(0);
    console.log(`Game 5:`);
    console.log(`  PGN: ${pgnOpening}${pgnVariation ? ": " + pgnVariation : ""} (${pgnEco})`);
    console.log(`  eco.json: ${result.opening.name} (${result.opening.eco})`);
    console.log(`  Walked back: ${result.movesBack} moves`);
  });
...
Qxd3 Qc4 29. Qd2 Be6 30. Nf5`;
    ...
    const result = findNearestOpening(longGame, openingBook, positionBook);
